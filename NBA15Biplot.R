...
df15<-fulldf[!(fulldf$Year < 2007 ),]
attach(df15)

ordf15 <- df15[order(Rank),]
ordf15
sum(MVP > 0)
ordf15[7059,]
...
df15 <- ordf15[ ,c("PTS","FTA","X2PA","X3PA","TS.","AST","TOV","TRB","STL","GS")]    
colnames(df15) <- c('PTS','FTA','2PA','3PA','TS%','AST','TOV','TRB','STL','GS')
df15
...
df15_mean <- sapply(df15, mean)
df15_sd <- sapply(df15, sd)
df15_min <- sapply(df15, min)
df15_max <- sapply(df15, max)



df15_pca <-prcomp(x = df15,
                center = TRUE, 
                scale. = FALSE)
print(df15_pca)
summary(df15_pca) 

df15_pca$x[,2] <- df15_pca$x[,2]*-1

df15_pca$rotation[,2] <- df15_pca$rotation[,2]*-1
...
axis(side = 2,
     at = c(-6,-4,-2,0,2,4),
     labels = TRUE)
title(main = "Biplot for PCs 15 Year NBA Stats",
      line = 3,
      adj = .5)
...
points(x = df_pca$x[,1:2],
       pch = c(rep(20, times = 7059),
               rep(20, times = 204)),
       cex = .5,
       col = c(rep("gold", times=7059),
               rep("blue", times=204)))
